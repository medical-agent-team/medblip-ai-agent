...
사용자와 소통하며 건강 문진, 이미지 처리, 다음 에이전트로 작업 전달
"""
...
from tools.medblip_tool import MedBLIPTool
...
class AdminAgentState(TypedDict):
    ...
    medblip_analysis: Optional[str]
    tasks_for_next_agent: List[str]
...
class AdminAgent:
    ...
    - 사용자와의 대화 관리
    - 건강 문진 수행
    - 이미지 업로드 및 MedBLIP 분석
    - 다음 에이전트로 데이터 및 태스크 전달
    """
    ...
    def __init__(self, openai_api_key: Optional[str] = None):
        ...
        self.conversation_stages = [
            "greeting",           # 인사 및 서비스 소개
            "basic_info",        # 기본 정보 수집 (나이, 성별 등)
            "medical_history",   # 과거 병력 문진
            "current_symptoms",  # 현재 증상 문진
            "image_request",     # 이미지 업로드 요청
            "image_analysis",    # 이미지 분석 수행
            "data_preparation",  # 다음 에이전트로 전달할 데이터 준비
            "handoff"           # 다음 에이전트로 인계
        ]
